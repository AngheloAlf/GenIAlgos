...
void fillArray_shift1Left(size_t n, uint8_t arr[n], size_t bpp, const uint32_t width){
    for(size_t i = 0; i < n; ++i){
        size_t aux = ((size_t)(i*PIXELS_PER_BYTE(bpp)))%MAX_COLORS(bpp);
        uint8_t x = 0;
        for(size_t j = 0; j < PIXELS_PER_BYTE(bpp); ++j){
            x <<= bpp;
            x |= (aux + j + (((i/(width/PIXELS_PER_BYTE(bpp))))%MAX_COLORS(bpp)) )%MAX_COLORS(bpp);
        }
        arr[i] = x;
    }
}
...
int main(){
    ...
    uint8_t arr[WIDTH*HEIGHT/PIXELS_PER_BYTE(BPP)];
    fillArray_shift1Left(n, arr, bpp, width);
